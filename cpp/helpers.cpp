...
void decodeJSIString(QuickValue &jsiValue, std::string *out, StringEncoding encoding) {
 ...
 */
StringEncoding getEncodingFromArgs(jsi::Runtime &rt, CppArgs *args, int argIndex, StringEncoding defaultValue, bool allowUTF8) {
  if (argIndex >= args->size()) return defaultValue;

  std::string encoding = args->at(argIndex).stringValue;
  if (encoding == "utf8" && allowUTF8) return ENCODING_UTF8;
  if (encoding == "hex") return ENCODING_HEX;
  if (encoding == "base64") return ENCODING_BASE64;
  if (encoding == "base64url") return ENCODING_BASE64URL;
  return defaultValue;
}

jsi::Value returnStringOrArrayBuffer(jsi::Runtime &rt, std::string &input, QuickDataType &ouputType, StringEncoding &outputEncoding) {
  if (ouputType == STRING) {
    std::string encoded;
