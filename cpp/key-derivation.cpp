...
namespace rncryptopp::keyderivation {
  ...
  */
  ...
  void deriveHKDF(std::string *secret, std::string *salt, std::string *info, std::string *result) {
    HKDF<T_hash> hkdf;
  ...
  void hkdf(jsi::Runtime &rt, CppArgs *args, std::string *target, QuickDataType *targetType, StringEncoding *targetEncoding) {
    ...
    if (!isDataStringOrAB(args->at(1)))
      throw facebook::jsi::JSError(rt, "RNCryptopp: hkdf password is not a string or ArrayBuffer");
  ...
  */
  ...
  void derivePBKDF(std::string *secret, std::string *salt, int iterations, std::string *result) {
    T_func<T_hash> func;
    byte derived[T_hash::DIGESTSIZE];
    size_t derivedLen = sizeof(derived);
    byte unused = 0;
  ...
  void pbkdf12(jsi::Runtime &rt, CppArgs *args, std::string *target, QuickDataType *targetType, StringEncoding *targetEncoding) {
    ...
    if (!isDataStringOrAB(args->at(1)))
      throw facebook::jsi::JSError(rt, "RNCryptopp: pbkdf12 password is not a string or ArrayBuffer");
    ...
    // Derive
    if (!invokeWithHash<derivePKCS12_PBKDF_wrapper>()(hash, &pass, &salt, numIter, target))
  ...
  void pkcs5_pbkdf1(jsi::Runtime &rt, CppArgs *args, std::string *target, QuickDataType *targetType, StringEncoding *targetEncoding) {
    ...
    if (!isDataStringOrAB(args->at(1)))
      throw facebook::jsi::JSError(rt, "RNCryptopp: pkcs5_pbkdf1 password is not a string or ArrayBuffer");

    if (!isDataStringOrAB(args->at(2)))
      throw facebook::jsi::JSError(rt, "RNCryptopp: pkcs5_pbkdf1 salt is not a string or ArrayBuffer");
    ...
    if (!invokeWithHash<derivePKCS5_PBKDF1_wrapper>()(hash, &pass, &salt, numIter, target))
