...
QuickValue createIntegerQuickValue(int value) {
  return QuickValue{.dataType = INTEGER, .doubleOrIntValue = static_cast<double>(value)};
}
...
QuickValue createArrayBufferQuickValue(char *arrayBufferValue, size_t arrayBufferSize) {
  ...
  for (int i = 0; i < int(count); i++) {
    ...
    if (value.isNull() || value.isUndefined()) {
      ...
      if (obj.isArrayBuffer(rt)) {
        ...
        target->push_back(createArrayBufferQuickValue((char *)buf.data(rt), buf.size(rt)));
      }
    }
    else {
      target->push_back(createNullQuickValue());
...
bool isDataString(QuickValue &value) {
  if (value.dataType == STRING) return true;
  return false;
}

bool isDataStringOrAB(QuickValue &value) {
  if (value.dataType == STRING || value.dataType == ARRAY_BUFFER) return true;
  return false;
}

bool isDataInteger(QuickValue &value) {
  if (value.dataType == INTEGER) return true;
  return false;
}
} // namespace rncryptopp::jsiHelper
